...
Some parameters for the application can be edited in the {\tt Settings} menu: image gama, application language, display settings, etc.
These parameters are stored, and are used at next application launch, so set them once to fit your own purpose.
As SaisieMasqQT can edit both images and 3d point clouds, application has different behaviour, depending on which data you deal with. As a result, {\tt settings} menu, and help window dialog (F1), will change depending on the loaded data. Be careful about this!
