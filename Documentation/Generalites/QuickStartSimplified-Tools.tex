...
To get the source (you will need to installate the mercurial versionning system) type :
...
This section describe a significative modification that occured since end of $2012$. To decrease the
...
However, it is recommaned that new script are based on the universal command {\tt mm3d}.
...
mm3d : Allowed commands 
 ...
 Campari	 Interface to Apero, for compensation of heterogenous measures
...
If the first aergument is not an existing command, some indication is given to help to find 
...
On the other hand, if you type {\tt mm3d ascul} , as there is no command begining by {\tt ascul},
one will get all the command containg {\tt ascul} :
...
Finaly if arg is nor a prefix nor a subexpression of any command, it will be tested as posix regular 
expression :
...
\begin{itemize}
   ...
   \item a key association for computing the name of the \emph{a priori }localisation file;
         ...
         optionnaly it can contain a tag of type {\tt OrientationConique}  defining
...
In the Mur Saint Martin data set, you cand find two files:
...
with several point of view having "optimale" angles for stereoscopy, I did not take any
...
To compute a posteriori the structure of an acquisition, the first thing is to know,
for each potential master image, what would be the best set of secondary  image.
...
Although , for further use, it is not strictly necessary to understand what was done this can
never be bad \footnote{at least, this is the "philosophy" with free open source products} \dots 
...
criteria to define the best set : it must covers  all the direction arround the master image
(to avoid hidden part), it must contains images with the "good" angles for stereocopy, it
must avoid redundancy and have a minimal number of images (for efficienncy). A these criteria
are generally contradictory, {\tt AperoChImSecMM} propose an optimall set for each cardinal
...
These result will used implicitelly in the following automatic tools. It is also possible to use them explicitely
...
to drive the future tools, it can already be used now for those who only need a regular coarse 3D model.
...
Unnamed args : 
  ...
  * string :: {Orientation}
Named args : 
  * [Name=Visu] bool :: {Interactif Visualization (tuning purpose, programm will stop at breakpoint)}
...
Even when the full automatic matching will be avalaible, many use will need to have
is some circonstances a finer control of the process. This is the aim of these tool.
