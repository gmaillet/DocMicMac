...
This chapter described simplified tools that allow to make
computations without filling XML-files.
Of course they cannnot deal with all the situation that are
...
Sorry this tool do not exist, but it will come \dots
...
These sections decribe a set of basic tools offering  a simplified interface
...
\begin{itemize}
   ...
   \item {\tt \bf Fraser}  a radial model, with decentric and affine parameters
         ...
         the optionnal parameters {\tt LibAff} and {\tt LibDec} (def value true) can
...
\begin{itemize}
    ...
    \item  compute the orientations of the $17mm$ images,  in the same
           coordinates system that the already oriented  $10mm$ images;
\end{itemize}
...
This section describe {\tt AperiCloud} a simplified version of the
...
\begin{itemize}
   ...
   \item Second  arg : pattern specifying the directory where orientation are located:
...
\end{itemize}
        ...
        %  -    -    -    -    -    -    -     -
\subsubsection{Scene based orientaion with {\tt SBGlobBascule}}
...
\begin{itemize}
  ...
  \item optional args {\tt PostPlan=\_MasqPlan}  means that if image is {\tt IMGP4171.JPG} 
   ...
   \item  if there are several masak it will use all them for
...
user can of course do it with a text editor, altenertively he can, on Linux,
...
To have the full syntax, as usall :
...
\begin{itemize}
   \item  for rectification, the images are  generateed with $Z=Cste$ ;
   \item  for matching, the genarated grid represents $Z=f(x,y)$ ;
\end{itemize}
...
it wont work in architectural context, to make the orthophoto of a
verical wall in some ground coordinate systeme where $Z$ axis  is vertical.
...
Then we get the "rectified" image of figuree~\ref{FIG:PBRec:StMartin}. This
...
The meaning of the three first arg is simimar to {\tt SBGlobBascule}. The fourth 
arg specify the output xml file.
...
The generated XML file {\tt RepCorr.xml} contain the necessary information to
...
simply to give thise repair as optional argument to the program. For
example :
...
information is given, the programms will adopt a default strategy it
...
area selection require some semantic interpretation of the scene
and can only me made by, you, the user who only knows what he wants.
For example, in the Saint Martin data set, an automatic programm has no
...
is to create a mask on a rectified mosaic.  The tool Tarama allow
...
The meaning of the two first mandotary argument should be quite obvious now.
For the optionnal arguments :

\begin{itemize}
   \item  {\tt Zoom} indicate the resolution of the mosaic, is must be a power
          of $2$ as it is made using the image pyramid of {\tt MicMac};

   \item  {\tt Repere} indicate and optionnal repair created by {\tt RepLocBascule}
...
{\tt SBGlobBascule} (\ref{ScBas:Basc}), on can type :
...
Figure~\ref{FIG:Rectif:StMartin} show the result.
...
\subsubsection{General caraterisctics}
...
beging euclidean coordinates); these case occurs quite often when the
scene is relatively flat and the acquisition is made by photo acquired 
orthogonally to the main plane. The main use case are :
...
\begin{itemize}
    \item  first args is a enumerated value, specyfing the kind of matching required;
           the two possible value are :

     \begin{itemize}
          \item {\bf \tt Ortho} , for a matchin adapted to ortho photo generation;
          \item {\bf \tt UrbanMNE} , for a matchin adapted to urban digital elevation model;
...
parameter. For example the parameter {\tt SzW}, define the size of correlation
windows, its default value is
...
As usual these default value can be change explicitely, for example :
...
The use of {\tt Tawny} is quite simple because it suppose that
...
The problem with such formula is than it can lead to important  drift in radiometry.
So there is also a global polynom $R$ that is computed, this polynome is such that :
...
The user can control the radiometric equalizaion by specifying the 
...
Named args : 
  * [Name=DEq] INT :: {Degre of equalization (Def=1)}
  * [Name=DEq] INT :: {Degre of equalization (Def=1)}
  * [Name=DEqXY] Pt2di :: {Degre of equalization, if diff in X and Y}
  * [Name=AddCste] bool :: {Add unknown constant for equalization (Def=false)}
  * [Name=DegRap] INT :: {Degre of rappel to initial values, Def = 0}
  * [Name=DegRapXY] Pt2di :: {Degre of rappel to initial values, Def = 0}
...
The only mandatory argument it is the directory where the 
...
\begin{itemize}
  \item {\tt DEq} specifies the degree of polynoms $O_i$, the defalut value is
        $1$ , wich means that for each ortho image, $A_i,B_i,C_i$ are computed
        to satisfy according to least mean square the equation~\ref{Eq:Rad:Mon};

   \item {\tt DEqXY} specifie the case where the degree of $O_i$ are different in
         $x$ and $y$, if  {\tt DEqXY=[$D_X$,$D_Y$]}, the unknowm monoms will
         be $x^n,y^m$ such that $n\leq D_X$ , $m\leq D_Y$ ,   $n+m\leq Max(D_X,D_Y)$;

   \item {\tt AddCste} in this case an unknown consntant $K_i$ is add to each ortho $O_i$ and
          the equation ~\ref{Eq:Rad:PolIndiv} is replaced by ~\ref{Eq:Rad:PolIndivCste};
          in allmost every case, it is preferable to let the defaut value {\tt AddCste=false};
...
Note howevern that this data-set is surprisingly difficult to equalize for such a
small set. With many data set, the default parameters already give an acceptable result.
...
On many data set the defautlt values should be OK. However it happened with difficult data
set that all the measure were refused for some pair of images, which obvioulsy led to an error.
