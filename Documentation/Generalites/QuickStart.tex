...
executed, sequentially, as a command line. Of course the symbol substitution has been \UNCLEAR{opered}
...
\end{tabular}
\label{Hidden:part}
\caption{Images 1 et 2 : exemple of binary hidden part; Image 3 , gray function of hidden part; Image 4 : thresholded
...
\begin{itemize}
    \item  this is made at a final step of the computation, so the hidden part will not be reused in correlation,
           however they are computed because they are usefull for mosaicing (see~\ref{Porto}).
...
The figure ~\ref{Resul:Ortho:MM} present the input and output of this ortho-image generation.
