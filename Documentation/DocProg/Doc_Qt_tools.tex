...
All the data in {\tt SaisieMasqQT} is rendered in an OpenGL context. We use an ortho projection (see {\tt cImageGL::drawGradientBackground, glOrtho). The main container, after {\tt SaisieQtWindow}, is {\tt GLWidget} (derived from {\tt QGLWidget}).
...
Editing a mask, is done in {\tt cGLData::editImageMask} ({\tt cGLData.cpp}).

\subsubsection{3D Mode}

Editing a mask, is done in {\tt cGLData::editCloudMask} ({\tt cGLData.cpp}).
