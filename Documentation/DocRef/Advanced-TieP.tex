...
than this two command co-exist.
...
\begin{itemize}
   ...
   \item $NbI$ is the number of images that are still uncovered (see bellow the definition, initialy no image is covered),
        ...
        a tie point is partly covered, its benefit to the photogrammetry strenght of the block decrease.
...
\begin{itemize}
   ...
   \begin{itemize}
         ...
         \item  else, update the quality of  $Q$, according to formula~\ref{QualTieP} to take into account
                the fact that the number of uncovered images has decrease.
   \end{itemize}
\end{itemize}
...
strength of a bloc. The so named Von-Gruber \footnote{typically if the number of such point where $6$
their optimal distribution would be those of the Von-Gruber point} point aims to fill
...
\begin{itemize}
   ...
   \item  let  $VG_{i1,i2}$ be the set  of tie point to select  for $I_1,I_2$,
          ...
          from  previous iteration of these Von Gruber points).
...
\begin{itemize}
   ...
   \item as soon as the first image is processed,  some tie points are selected, and it must have an influence 
...
\begin{itemize}
   \item create a partition of the images $P=\{S_1,S_2,...S_N\}$ such that
         $\forall i,j,n$  with $I_i \in S_n$ and $I_j \in S_n$ then $ TieP(I_i,I_j) = \emptyset $; in fact this condition
        is not fixed strictly, and it is accepted that the intersection be equal to a very few percentage of the points;
   ...
   \begin{itemize}
        ...
        \begin{itemize}
              ...
              \item do the same computation as with red tie points
