...
\begin{itemize}
    \item it must constain a string $K$ which describes  a key association,
...
is a specific  tranformation between the photo and the scanner.
...
iamges: the images have been randomly rotated and scaled, simulating the
...
matching: the {\tt <OrIntImaM2C>}  section contains the affinity betwwen scanner and image computed
...
Kugelhopf (Klics Ubuesques Grandement Evites, Lent, Hasardeux mais Utilisable pour Points Fiduciaux) is a tool for scanned images. It is used to automatically find fiducial marks on the images.
...
The output is xml files in {\tt Ori-InterneScan} for every picture where the automatic search was successfull.
